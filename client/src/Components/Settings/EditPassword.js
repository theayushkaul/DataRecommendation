...
export default function EditPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await axios.put(`/user/update/${user._id}`, { password }, config);
            console.log(res);
    ...
    return (
        <div>
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} style={{
                content: {
                    top:"35%",
                    bottom:"35%",
                    right:"38%",
                ...
                <form className="flex flex-col mt-6" onSubmit={(e)=>handleSubmit(e)}>
                    <label className='text-xl'>Edit Password</label>
                    <input className="focus:outline-none rounded-full px-4 mt-6 text-sm py-1" style={{backgroundColor:"rgba(0,0,0,0.8)"}} type="text" placeholder="Enter your new password..." onChange={(e) => setPassword(e.target.value)} />
